...
knitr::opts_knit$set(
  root.dir = "C:/Users/simen/Desktop/tv2tourmanager"
)
```
...
top_15_abs <- cleaned_data |> 
  ...
  theme_minimal() +
  labs(
    title = "Top 15 highest ranked players",
    y = NULL,
    x = "Points"
  ) +
  scale_x_continuous(
...
top_15_rel <- cleaned_data |> 
  ...
  labs(
    title = "Top 15 highest ranked players",
    subtitle = "Points per price unit", 
    y = NULL,
    x = "Points per price unit"
  ) +
  scale_x_continuous(
...
plot_top_15 <- function(
  ...
  plot <- dataframe |> 
    ...
    labs(
      title = {{ position }},
      x = ifelse(absolute_points == FALSE, "Points", "Points per price unit"),
      y = NULL
    ) +
    scale_x_continuous(
...
p1 <- cleaned_data |> 
  ...
  labs(
    title = "Which manager should you use?",
    subtitle = "Absolute points",
    x = "Points",
    y = NULL
  )
...
p2 <- cleaned_data |> 
  ...
  labs(
    title = "Which manager should you use?",
    subtitle = "Relative points, per price unit",
    x = "Points per price unit",
    y = NULL
  )
```
